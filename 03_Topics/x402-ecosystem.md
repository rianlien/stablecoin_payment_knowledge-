...
チェーン拡張
  ...
  └ Canton Network（Goldman Sachs・SIX Group 等参加の機関向け Daml 環境、2026-06-15）
```
...
| 2026-06-19 | [[2026-06-19_AllUnity_sekau-launch-agentic-payments]] | AllUnity が SEKAU（SEK 建て MiCAR 準拠 EMT）と x402 Agentic Payments を正式ローンチ——EU non-USD 圏での x402 本番稼働・ステーブルコイン保有不要の銀行精算設計 |
...
| 2026-06-15 | [[2026-06-15_x402_canton-network-exact-scheme]] | Canton Network（Goldman Sachs・SIX Group 等参加の機関向け Daml ブロックチェーン）の exact scheme 対応を追加——EVM 以外の機関向け環境で x402 machine payments が利用可能に |
| 2026-06-17 | [[2026-06-17_x402_onpaymentrequired-trust-gate]] | x402 クライアントに `onPaymentRequired` trust-gate フック追加——支払い前のペイアドレス検証・支出上限・外部信頼スコアリングを標準化（フィッシング対策） |
| 2026-06-19 | [[2026-06-19_x402_siwx-go-support]] | SIWX（Sign-In-With-X）が Go SDK に対応——TypeScript↔Go クロス SDK 互換性確認済み、エンタープライズ向け認証型アクセスが完成 |
...
| 2026-05-11 | [[2026-05-11_Cryptorefills_x402-agent-checkout-mcp]] | Cryptorefills が x402 決済を EC チェックアウトに実装——MCP サーバー + オープンソース agentic-commerce 参照実装も公開 |
| 2026-05-11 | [[2026-05-11_Cryptorefills_x402-ai-agents-checkout]] | Cryptorefills が x402 決済を AI エージェント向けに提供開始——180 か国・6,600 ブランドのギフトカード等をワンラウンドトリップで購入可能 |
...
### 機関向けブロックチェーン（Canton Network）
2026-06-15 に Canton Network（Goldman Sachs・SIX Group・BNP Paribas・Citi 等参加の Daml ベース機関向けブロックチェーン）への exact scheme 対応が追加。EVM 非依存のプライベートブロックチェーンで x402 が動作し、TradFi 機関がエージェント決済エコシステムに参加するルートが開かれた。ファシリテーターは読み取り専用アクセスで決済検証するため、既存の Canton コンプライアンス・プライバシーポリシーとの衝突が最小化される。詳細 → [[2026-06-15_x402_canton-network-exact-scheme]]

### セキュリティ拡張（trust-gate フック）
2026-06-17 に x402 クライアントミドルウェアに optional な非同期コールバック `onPaymentRequired` が追加。支払い実行前にペイアドレス検証・支出上限・外部信頼スコアリングを適用できる標準拡張点。x402-kit フィッシングキャンペーンや V2 動的ペイアドレスルーティングへの対応として設計。詳細 → [[2026-06-17_x402_onpaymentrequired-trust-gate]]

### SIWX マルチ SDK 対応（TypeScript・Go）
2026-06-19 に Go SDK が SIWX（Sign-In-With-X）に対応し、TypeScript↔Go のクロス SDK 互換性が Base Sepolia で確認済み。エージェントが一度認証すれば繰り返しアクセス時の都度課金を回避できる認証型アクセスが多言語スタックで実現した。詳細 → [[2026-06-19_x402_siwx-go-support]]
...
- Canton Network 参加機関（Goldman Sachs 等）による x402 本番採用アナウンス
- trust-gate フック（onPaymentRequired）採用率と外部信頼スコアリングサービスの登場
- SIWX の Python SDK 対応完了と三言語クロス SDK 互換テスト
...
- [[Chainalysis]] — x402 採用状況の定量分析（累計 1 億件報告）
