...
## Output count
- 1回の実行で作成する新規ニュースノートは最大10件（CLAUDE.md に準拠）、原則5件以下を目安にする
...
- `entity` は必ずスカラー文字列で書く。**空欄不可**。記事の主体となる企業・機関・ネットワーク名を記入する。複数の場合は `"Company A / Company B"` 形式とし、YAML リスト形式（`entity:\n  - ...`）は使わない
- `primary_category` は単一カテゴリのみを書く（例: `regulation`）。`/` 区切りや複合値は不可（NG 例: `"regulation / stablecoin-issuer"`）。複数カテゴリは `category` フィールドに `/` 区切りで書く
...
リンク形式は `[Source Name（YYYY-MM-DD）](URL)` を厳守する。
- source に公開日がない（プロダクトページ・GitHub リポジトリ等）場合は `underlying_event_date` を代わりに使う
- 日付が完全に不明な場合のみ `（日付不明）` と書く（Slack スクリプトはリンク本体を解析するため形式は崩さない）
